...
describe('TimeBlockTemplatesService', () => {
  ...
  describe('update', () => {
    ...
    it('should validate time range when updating start time only', async () => {
      ...
      await expect(service.update('tpl-123', 'user-123', { startTime: '09:00' })).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should validate time range when updating end time only', async () => {
      ...
      await expect(service.update('tpl-123', 'user-123', { endTime: '06:00' })).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe('deactivate', () => {
    ...
    it('should deactivate template with provided activeUntil date', async () => {
      ...
      const deactivatedTemplate = {
        ...mockTemplate,
        isActive: false,
        activeUntil: new Date('2024-02-01'),
      };
    ...
    it('should throw NotFoundException when template not found', async () => {
      ...
      await expect(service.deactivate('nonexistent', 'user-123', {})).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('materializeForDateRange', () => {
    ...
    it('should do nothing when user has no active templates', async () => {
      ...
      await service.materializeForDateRange(
        'user-123',
        utcDate('2024-01-15'),
        utcDate('2024-01-21'),
      );
