...
export class TimeBlockTemplatesService {
  ...
  async deactivate(id: string, userId: string, dto: DeactivateTemplateDto) {
    ...
    const activeUntil = dto.activeUntil ? new Date(dto.activeUntil) : new Date();
  ...
  async materializeForDateRange(userId: string, startDate: Date, endDate: Date): Promise<void> {
    ...
    const existingSet = new Set(
      existingBlocks.map((b) => `${b.templateId}:${b.day.date.toISOString().split('T')[0]}`),
    );
    const exclusionSet = new Set(
      exclusions.map((e) => `${e.templateId}:${e.date.toISOString().split('T')[0]}`),
    );
