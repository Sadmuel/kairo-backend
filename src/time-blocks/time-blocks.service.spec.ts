...
describe('TimeBlocksService', () => {
  ...
  describe('duplicate', () => {
    ...
    it('should throw NotFoundException when source time block not found', async () => {
      ...
      await expect(service.duplicate('nonexistent', 'user-123', baseDuplicateDto)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should throw NotFoundException when source belongs to different user', async () => {
      ...
      await expect(service.duplicate('tb-123', 'user-123', baseDuplicateDto)).rejects.toThrow(
        NotFoundException,
      );
    });
